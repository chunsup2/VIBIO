...
FIRST_COL = "Dataset proporation"          # Column A header

# def update_results_csv(csv_path: str,
#                        cls_type: str,
#                        proportion: float,
#                        value):
#     prop_col = f"{proportion:.3g}"          # Column header (e.g., 0.05 / 0.1)
#     csv_file = Path(csv_path)

#     # ---------- 1. Read file (create new if file doesn't exist or is empty) ----------
#     if csv_file.exists() and csv_file.stat().st_size > 0:
#         df = pd.read_csv(csv_file)
#     else:
#         df = pd.DataFrame(columns=[FIRST_COL])

#     # ---------- 2. If no proportion column exists ⇒ add new ----------
#     if prop_col not in df.columns:
#         df[prop_col] = np.nan               # Add column directly at the end

#     # ---------- 3. Find/create cls_type row ----------
#     same_cls = df[FIRST_COL] == cls_type
#     empty_rows = df.index[same_cls & df[prop_col].isna()].tolist()

#     if empty_rows:                          # Has empty slots
#         target_row = empty_rows[0]
#     else:                                   # No empty slots ⇒ append new row
#         new_row = {FIRST_COL: cls_type}
#         df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
#         target_row = df.index[-1]

#     # ---------- 4. Write value and save ----------
#     df.at[target_row, prop_col] = value
...
def update_results_csv(csv_path: str,
    ...
    追加/更新结果到 CSV：
    - AUC 写入列  <prop_col>
    - AUC 的 bootstrap std（若提供）写入列 <prop_col>_std
    其中 prop_col = f"{proportion:.3g}" 例如 "0.05" / "0.1" / "0.5"
    """
    prop_col = f"{proportion:.3g}"
    std_col  = f"{prop_col}{std_suffix}"
    csv_file = Path(csv_path)
    ...
    if prop_col not in df.columns:
        df[prop_col] = np.nan
    if std is not None and std_col not in df.columns:
        df[std_col] = np.nan

    # 3) 找/建目标行（对应 cls_type）
