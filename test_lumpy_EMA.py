...
def build_models(args, device):
    ...
    ckpt = os.path.join(
        ...
        args.srtype,
        f"ema{args.cls_type}_{args.train_type}_{args.kl}_{args.lr}.pth"
    )
...
def main():
    ...
    wandb.init(
        project='VIB-MRI',
        name=f"Test_{args.proporation}_{args.cls_type}_{args.train_type}_{args.data}_{args.kl}",
        config=vars(args),
    ...
    # test_ds = MRIDataset(args.test_image_path, proportion=1.0)
    ...
    mu0_mean = np.load(os.path.join(npy_path, f"{args.cls_type}_{args.train_type}_kl{args.kl}_lr{args.lr}_mu_ema.npy"))
    s = np.load(os.path.join(npy_path, f"{args.cls_type}_{args.train_type}_kl{args.kl}_lr{args.lr}_s_ema.npy"))
    Kinv = np.load(os.path.join(npy_path, f"{args.cls_type}_{args.train_type}_kl{args.kl}_lr{args.lr}_Kinv_ema.npy"))
    ...
    mu_all = np.zeros((1, 10))
    label_all = np.zeros((1, 1))

    with torch.no_grad():
        for i, (imgs, labels) in tqdm(enumerate(test_loader), total=len(test_loader)):
            imgs = imgs.to(device)
    ...
    csv_path = f"results/{args.data}_p{args.proporation}_lr{args.lr}_kl{args.kl}_vibio.csv"
